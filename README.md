...
卡功能管理使用 GSMA SGP.22 ES10c，通过 ML307R 的逻辑通道与 APDU 读取及启用 Profile。后端只接受刚从 eUICC 枚举得到的 Profile AID，不开放任意切卡 APDU。切换会重启 4G 模组、恢复自动选网并等待完整注册结果，短信与蜂窝网络通常中断 30–180 秒，漫游注册通常需要 1–5 分钟，异常恢复最长约 8 分钟；任务状态会持久化，并在重启后继续核对实际结果。
